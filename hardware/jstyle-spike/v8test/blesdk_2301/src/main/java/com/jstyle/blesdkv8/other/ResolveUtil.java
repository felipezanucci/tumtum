package com.jstyle.blesdkv8.other;
...
import com.jstyle.blesdkv8.model.DeviceBean;
...
public class ResolveUtil {
    ...
    public static  Map<String,Object> getDeviceTime(byte[] value) {
        ...
        mapData.put(DeviceKey.Week, week+"");
        mapData.put(DeviceKey.MTUlength, mtu+"");
        mapData.put(DeviceKey.GPSTime, gpsDate);
        maps.put(DeviceKey.Data, mapData);
        return maps;
    }

    public static  Map<String,Object> Temperature_3NTC(byte[] value) {
        Map<String,Object> maps=new HashMap<>();
        maps.put(DeviceKey.DataType, BleConst.Temperature_3NTC);
        maps.put(DeviceKey.End,true);
        Map<String,String>mapData=new HashMap<>();
        int temp = getValue(value[1], 0)+getValue(value[2],1);
    ...
    public static Map<String,Object> getPPG(byte[] bleData) {
        Map<String,Object> maps=new HashMap<>();
        ...
        int length = bleData.length;

// packetID 只取 1 字节，注意 & 0xFF 防止符号扩展
        int packetID = bleData[1] & 0xFF;
        ...
        for (int i = 0; i < count; i++) {
            ...
            int value =
                    (bleData[index] & 0xFF) |
                            ((bleData[index + 1] & 0xFF) << 8) |
                            ((bleData[index + 2] & 0xFF) << 16);
            pp.append(value).append(i==(count-1)?"": ",");
        }

        mapData.put(DeviceKey.arrayPpgRawData, pp.toString());
        mapData.put(DeviceKey.packetID, packetID+"");
        return maps;
    }
    ...
    public static Map<String,Object> GetBasic_parameters_of_equipment(byte[] value) {
        Map<String,Object> maps=new HashMap<>();
        maps.put(DeviceKey.DataType, BleConst.GetBasic_parameters_of_equipment);
        maps.put(DeviceKey.End,true);
        Map<String,String>mapData=new HashMap<>();
        maps.put(DeviceKey.Data, mapData);
        mapData.put(DeviceKey.Is_the_sports_mode_flashing, getValue(value[5],0)==0?"0":"1");
        mapData.put(DeviceKey.Is_the_light_flashing_when_the_Heart_Rate_is_too_high_in_sports_mode, getValue(value[6],0)==0?"0":"1");
        return maps;
    } public static Map<String,Object> PPG(byte[] value) {
        Map<String,Object> maps=new HashMap<>();
        maps.put(DeviceKey.DataType, BleConst.PPG);
        maps.put(DeviceKey.End,false);
        Map<String,String>mapData=new HashMap<>();
        maps.put(DeviceKey.Data, mapData);
        mapData.put(DeviceKey.Is_the_sports_mode_flashing, getValue(value[5],0)==0?"0":"1");
    ...
    public static Map<String,Object> DeletesLEEP() {
        Map<String,Object> maps=new HashMap<>();
        maps.put(DeviceKey.DataType, BleConst.DeleteSleep);
        maps.put(DeviceKey.End,true);
        List<Map<String, String>> list = new ArrayList<>();
        maps.put(DeviceKey.Data, list);
        return maps;
    ...
    public static Map<String,Object> getObtainDetailedSleepData(byte[] value) {
        Map<String,Object> maps=new HashMap<>();
        maps.put(DeviceKey.DataType, BleConst.Obtain_detailed_sleep_data);
        maps.put(DeviceKey.End,false);
        List<Map<String, String>> list = new ArrayList<>();
        maps.put(DeviceKey.Data, list);
        int count = 130;
        ...
        for (int i = 0; i < size; i++) {
            ...
            String date ="20"+ByteToHexString(value[3 + i * count]) + "."
                    + ByteToHexString(value[4 + i * count]) + "." + ByteToHexString(value[5 + i * count]) + " "
                    + ByteToHexString(value[6 + i * count]) + ":" + ByteToHexString(value[7 + i * count]) + ":" + ByteToHexString(value[8 + i * count]);
            int SleepLenth=getValue(value[9 + i * count],0);
            byte[] bs = new byte[Math.min(SleepLenth, 60)];
            byte[] bs2 = new byte[Math.min(SleepLenth, 60)];
            System.arraycopy(value, 10, bs, 0, bs.length);
            System.arraycopy(value, 70, bs2, 0, bs2.length);
            String A[]=byte2Hex(bs).split("-");
            String B[]=byte2Hex(bs2).split("-");

          List<String> Sleep_level=new ArrayList<>();
          List<String> ActivityData=new ArrayList<>();
          for (String aa:A){
              String part1 = aa.substring(0, 1); // 获取第一个字符
              String part2 = aa.substring(1);    // 获取剩余部分
              Sleep_level.add(part1);
              Sleep_level.add(part2);
          }
            for (String aa:B){
                String part1 = aa.substring(0, 1); // 获取第一个字符
                String part2 = aa.substring(1);    // 获取剩余部分
                ActivityData.add(part1);
                ActivityData.add(part2);
            }

            hashMap.put(DeviceKey.Date, date);
            hashMap.put(DeviceKey.KSleepLength, SleepLenth+"");
            hashMap.put(DeviceKey.Sleep_level, Arrays.toString(Sleep_level.toArray()));
            hashMap.put(DeviceKey.ActivityData, Arrays.toString(ActivityData.toArray()));
            list.add(hashMap);
        }
...
/*    public static void Getecgdata(byte[] value,Databack databack) {
    ...
    public static Map<String,Object> setTimeSuccessful(byte[] value) {//设置时间成功后返回手机一个数据包的最大长度
        ...
        Map<String, String> map = new HashMap<>();
        maps.put(DeviceKey.Data, map);
        map.put(DeviceKey.MTUlength, String.valueOf(getValue(value[1],0)));
        return maps;
    }
