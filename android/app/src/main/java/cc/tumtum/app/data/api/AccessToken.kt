 ...
 * end of a six-hour capture, with no way back to a login screen. That happened
 * to the capture app on 2026-08-27, one day after signing in, and left it
 * holding readings it could never upload. Ported from there on 2026-09-18.
 *
...
object AccessToken {
    ...
    /**
     * A token whose expiry cannot be read is treated as live: the server is
     * the authority, and refusing a token we merely failed to parse would lock
     * somebody out of an account that works.
     */
    ...
    /** Millis left before the token dies, or null when there is no readable expiry. */
