...
11. ~~**Password reset does not exist.**~~ **24/09: it had never sent a
    mail in production** — `RESEND_API_KEY` was never set on Railway. **Set
    by Felipe the same evening; the first reset mail arrived**, and #98
    (the 6-digit sign-up code, which needs the same key) was unblocked.
    Built 2026-08-26 on Resend, via
...
52. ~~**The OPERADOR toggle is a UI gate, not a permission.**~~ **Closed
    25/09:** the app stores `is_admin` from `/api/auth/me` against the
    account, and the marks and event shortcuts need the switch *and* the
    server's word (entry of 25/09). *(Original:)* Noticed 22/09
...
60. ~~**Tests 6–10 of the 23/09 round are still to run on b173**~~ **All run
    by 25/09.** 8 and 9 passed in full on b195; 7b's question (another
    account's night on this phone) became Felipe's rule that such a night is
    hidden, built the same day. *(History:)* ~~the session
    ...
    **The steps were never written down; sent to Felipe 24/09, in short:**
    **(7)** a brand-new account opens an event's feed → *"Esse feed é de quem
    estava lá. Sua noite não chegou aqui."*; the card of a night uploaded
    after 23/09 by another account says *"Essa noite foi enviada por outra
    conta"* with no post button; a night from before 23/09 (no owner stored)
    offers the button and shows the server's sentence in red, never *"Não deu
    pra mostrar agora"*. **(8)** the operator account (the server refuses a
    fan's marks), a JOGO started with the strap off, APITO and two GOL visible
    on `/admin/eventos` **before** ending the night; one mark tapped in
    airplane mode goes up with the next tap; the marks survive ending a night
    with no readings. **(9)** best before 10h, since a night that ends before
    10h opens at 10h the same day: lock on, a short capture with the strap,
    *"Sua noite está sendo revelada."*, LACRADA in Você, the feed's *"Sua
    noite abre às 10:00"*, the curve appearing at 10h on its own, the
    *"Sua noite abriu."* notification (inexact by minutes), then the lock
    turned **off**. Test 8 exposed one gap worth keeping: the capture screen
    never says whether a mark reached the server, only *"marcado às"*.
    **24/09, in a hand: 7a passed** — a new account on the TesteMadonna feed
    read exactly *"Esse feed é de quem estava lá. Sua noite não chegou
    aqui."* **7b and 7c could not run, and the steps were the reason**: they
    had the test account made with *Criar outra conta*, which wipes the
    phone's nights (the screen says so; the 18/09 entry says why), and then
    asked for one of those nights. **7b, redone:** with the test account, a
    short capture at an event (the night uploads as the test account); then
    *Entrar* — never *Criar* — with the usual account, which keeps the
    phone's nights; that night's card must say *"Essa noite foi enviada por
    outra conta"* with no post button. **7c is gone for good:** it needed a
    night from before 23/09 with no owner, and those left the phone with the
    wipe; every newer night has an owner.
61. ~~**The share menu's layout has a hole (Felipe, b187, 24/09).**~~
    **Built the same day** (entry "the share menu: Pink, no hole, and
    Status"): `ShareGrid` pairs the networks and gives a lone one the whole
    row. On a phone: not yet. *(Original:)* The
...
62. ~~**The share buttons in Pink, for emphasis (Felipe, 24/09).**~~ **Built
    the same day**: the networks are Pink with black type; Copiar, Salvar and
    Mais apps are small outlined chips beneath. On a phone: not yet.
    *(Original:)* Today they are
...
63. ~~**WhatsApp Status as its own destination (Felipe, 24/09).**~~
    **Confirmed on b189 the same evening:** Felipe opened Status from the
    button, posted, and saw the post from another phone. The footnote stays,
    for WhatsApp versions that ignore the extra. *(As built:)* *Status do WhatsApp* sends
    the finished file with `jid` = `status@broadcast`. Nothing comes back to
    say whether WhatsApp honoured it, so the screen says both roads before the
    tap (*"se abrir a lista de conversas, toca em Meu status, lá no topo"*);
    an outright refusal opens the chat picker and says so. **To learn on
    Felipe's phone:** whether the extra opens Status, and whether the chat
    picker's first row really reads *Meu status* in his WhatsApp.
    *(Original:)* Today
...
64. ~~**An e-mail that does not exist makes an account (Felipe, 24/09).**~~
    **Built the same day, at sign-up** (Felipe: *"vamos mandar o código de 6
    ...
    falsas"*). See the entry "an account needs its e-mail". **Proved in a
    hand 25/09**, on the phone and on the site, codes arriving both times. *(Analysis, as written before the call:)*
    ...
    `@`), and `teste@teste.com` is well-formed. The only proof that an
    address exists and is the person's is **a message they receive**: a
    6-digit code by e-mail at sign-up, typed into the app. Resend already
    sends the password reset (`services/email.py`), so the mail half exists;
    whether `RESEND_API_KEY` is set on Railway has to be checked, since
    without it no code arrives and nobody could sign up. **The decision is
    where it blocks:** before the account is usable at all (cleanest, but
    the first run then depends on mail delivery and the spam folder), or
    only before the first public act, posting to a feed (softer, keeps
    capture working). And one constraint that decides the shape: the
    deployed server never runs Alembic (item 16), so "confirmed" must live
    in **a new table**, never a new column on `users`, which would silently
    not exist in production. Existing accounts: confirmed at their next
    sign-in, or grandfathered. Costed at about one session.
65. **A night never comes back from the server** ~~, and a second account
    wipes the phone~~. **The wipe is gone (25/09):** each night belongs to
    the account that recorded it, another account's are hidden and come back
    with it, and creating an account deletes nothing. What remains is the
    download, below. *(Original:)* Known since 18/09 ("nothing downloads a night yet"), and it
    cost Felipe every night on his phone on 24/09 (test 7). The nights are
    safe on the server, under the account that uploaded each one; the app
    has no road to bring them back, so a reinstall, a new phone or *Criar
    outra conta* empties *Você*, although the server keeps
    `GET /api/health/sessions` and `/api/experience/{id}`. **The wipe's own
    reason is gone:** on 18/09 nights had no owner, so a second account
    would have seen someone else's; since 23/09 every night records its
    owner (Room v8, `ownerUserId`). Two changes follow, each on its own:
    stop wiping and show each account its own nights, and bring an
    account's nights down from the server on sign-in. The second is what a
    fan with a new phone needs.
66. **Fake wearable connections in the database.** Until 25/09 the site's
    welcome "connected" a watch by saving a `wearable_connections` row with
    the token `placeholder-token`, and the site profile lists those rows as
    *"Dispositivos conectados"*. The page is fixed; the rows are still
    there. Count them and delete them, then decide whether the profile
...
69. ~~**The skin-contact filter is reasoned, not measured.**~~ **Measured
    ...
    (`BeatFilter`). **Proved in a hand on b201/b204**: setup says "sem
    contato" for the strap on the table, the capture says SEM CONTATO with no
    number, and the night breaks its line where the strap was off.
...
### b201 in a hand, the same afternoon

- **Test 1 passed.** *"A curva abre aqui amanhã às 10h00"*, the Você row
  with *amanhã*, and the hidden-nights line for several accounts.
- **Test 2 passed: the first half of item 69 is proved.**
  - Worn: *Pronto, conexão feita*, in Pink, with 68.
  - On the table: *"Sensor pareado. … sem contato com a pele"*.
  - Worn again: *Pronto* with 92.

  The line said *"está ligado, mas sem contato"*, which stops being true
  twenty seconds later, when the strap switches itself off on the table.
  It now says only *"está sem contato com a pele"*.
- **The yellow went back to white.** Seeing it on the phone, Felipe: *"o
  que está em amarelo volta para branco… pode deixar só o botão em rosa"*.
  The strap road's label is white and its button a white outline, and Pink
  stays the only colour on the screen. So the second explosion was one too
  many on a screen whose job is a single decision.
- **Test 3 passed on the live screen, and the night showed one more lie.**
  - Worn: 74, *OK*.
  - Off, at 00:00:40: the number gone, and *POLAR H10 · SEM CONTATO* in
    Pink. The zero of b198 never appeared.
  - At 00:01:03 the strap had switched itself off: *RECONECTANDO*.
  - Worn again at 00:01:31: 67, *OK*.
  - **The night was wrong.** The strap was off for about fifty seconds, and
    the curve drew a straight line from the last beat before to the first
    beat after, across the silence. The line broke only after **60 s**
    without a reading, a threshold chosen for a watch that reads once a
    minute. For a 1 Hz strap, fifty seconds of nothing is a gap. **The
    threshold now follows the source:** 10 s when the median interval is 2 s
    or less, 60 s otherwise. The same applies to the phone's curve, the
    card's curve, the gap marks and coverage (which was counting the
    silence as covered).
  - Two smaller things on the same screen:
    - The counter kept climbing with the strap on the table (40 → 57
      *AMOSTRAS*). It now counts beats, not packets.
    - The absent number was a large Pink bar, the "—" in the hero font. It
      is now a dim grey dash; the header already says why.
  - Not checked: whether the night's 81, its peak, right where the strap
    came off, was a real beat or the strap being pulled. Night 18 had one
    such reading, a 252 ms R-R under a bpm of 71. The export of this night
    would tell.

### b204 in a hand: two roads to one strap

- **Test 1 passed by AO VIVO.** The strap road is white, only *Procurar de
  novo* is Pink, and *Pronto* shows 67 while the strap is worn.
- **Felipe went in by another door first.** After REMOVER he tapped
  Configurações' own Pink *Conectar sensor* and got a different screen, the
  old white bottom sheet *Buscando sensores*. He read it as broken. It was
  not broken, but it was wrong:
  - It was a second way to pair a strap, and it **paired on the tap**, with
    no beat waited for and no "sem contato". The strap on the table, fixed
    this afternoon on one road, could still be paired from the other.
  - His rule: *"nos dois caminhos, a experiência tem que ser igual"*.
- **There is one road now.** *Conectar sensor* and *TROCAR* in Configurações
  open the setup screen already searching (`Routes.SensorSearch`), and it
  ends in the same proof of a person. *Parar* goes back to Configurações.
  The old sheet is deleted.
- The lesson is the old one in another shape. **When a fix lands on one
  screen, look for every other screen that does the same act.** The strap
  was paired in two places, and only one of them had been fixed.
- **Test 2 passed: the second half of item 69 is proved in a hand.**
  - Off the skin: *SEM CONTATO*, a dim dash, and the counter stopped at
    67.
  - Back on: 68, and the counter moved on to 75 a minute later.
  - The night: the line stops where the strap came off, a dashed base with
    *1 MIN SEM DADO*, and the line starts again where it went back on.
    There was no straight line and no zero.

  (The capture showed MÚSICA/MOMENTO and the night an export button, both
  operator-only. The test ran on the operator account, not on +teste13.)
- **One more claim under the curve.** The right-hand time was the *peak's*,
  in yellow, sitting at the end of the axis under a dot drawn at the start:
  *15H19 … 15H19*, a night that began and ended in the same minute. The
  axis now ends with the night's end time, in grey. The peak's time is
  already written next to its number.

---

## 2026-09-25 — b196 in a hand: every test passed, and the app still said too much

Felipe ran the seven tests of b196 on 25/09, from 11:08 to 12:20, and asked
again for the findings to be collected and fixed together. **All seven
passed** in what they set out to check:

- another account's night hidden, and back with its account;
- a signed-out phone refusing to record;
- a new account wiping nothing and starting without the previous sensor;
- a night captured on the Polar, uploaded and analysed;
- a night recorded offline, which the event feed then called "neste
  celular, mas ainda não subiu" and sent with one tap;
- the operator's "só neste celular" line naming the cause and clearing
  itself on TENTAR DE NOVO;
- the site's welcome telling the truth.

One check never ran: the sealed night's Pink *Voltar pro início*. The reveal
lock was already off.

**What the round found was the app claiming things about who and what is
there:**

1. **Signed out, it still read as Felipe's.** The FZ avatar sat on every
   tab. The profile said *"Felipe Zanucci, Editar perfil"*, and
   Configurações showed his name and photo: *"tudo dá a entender de que
   eu ainda estou na minha conta"*. b196 had made it worse on purpose,
   keeping the last account's nights visible after Sair. His model wins:
   **signed out, the phone is nobody's.**
2. **A refusal said off screen.** Tapping a live event signed out did
   nothing visible. The sentence was right but sat under the list, behind
   the tab bar, and he read it as "não acontece nada".
3. **A strap on a table measured a heart rate.** The Polar H10 was found
   unworn, and setup showed *"Pronto, conexão feita"* with a number. The
   sensor reports skin contact in every packet, and the app stored it and
   never read it. The same hole ran through the capture: a strap taken off
   mid-show would have drawn beats nobody had into the night and onto the
   server. He confirmed the strap lay untouched, so the number was false.
4. **Two rows about two devices.** "Nenhum relógio conectado" in grey sat
   above a ready Polar. His call on the product behind it: *"o usuário
   final mesmo, praticamente todo mundo vai usar só relógio"*. The watch
   leads setup, and the strap is the quiet second road.
5. **A list row that started recording.** He did not expect a tap on an
   event to start a capture, and asked which was better. Decided: a row
   opens a start sheet, which says what will record (or what is missing)
   and holds one Pink action.
6. **The experiment's participant code**, in sign-up and in Configurações.
   His word: *"não faz sentido, a gente não vai precisar"*.
7. **Small claims:** *"1 NOITES"*; *"Momentos encontrados pelo servidor"*
...
### What b198 builds (green on the first build)

- **Nobody is shown when signed out.**
  - The viewer is the signed-in account only. The last account is kept to
    own a night whose session ended mid-capture, and for nothing else.
  - Every tab's corner is `AccountCorner`: the avatar, or a Pink ENTRAR.
  - Você, the gallery and the profile ask to sign in.
  - Configurações hides the profile and the delete button.
- **The start sheet.** A live event's row opens it, with three states:
  - signed out: the account first;
  - nothing connected: *Conectar meu relógio*, or *Gravar assim mesmo*;
  - ready: *Vai gravar com …* and *Começar a gravar*.

  The marked event's own *Começar agora* stays one tap when everything is
  in place.
- **`SkinContact`.** A reading marked "no contact" is filtered out of the
  night, the live number and the upload; the raw rows stay for the export.
  The capture header says **SEM CONTATO**. Setup says *"ligado, mas sem
  contato com a pele"* and never *"Pronto"* for it.
- **Watch-first setup.**
  - The Health Connect watches that wrote heart rate in the last 24 hours
    are listed, the first *Usar* in Pink. When there is none, a line on
    what makes a watch appear and *Procurar de novo*.
  - *Pronto, relógio conectado* once one is chosen.
  - The strap sits under *Tem uma cinta de peito?*.
  - AO VIVO has one status row, and signed out it shows only the sign-in
    block and the events.
- **Smaller fixes.**
  - The participant code is removed, and the export is operator-only.
  - The crowd counts are plurals.
  - *"UMA NOITE TRANQUILA."* and *"O servidor não achou nenhum momento
    nessa noite"* when there is none.
  - The Configurações scan sheet gets *Procurar de novo* when Bluetooth
    is off.
  - The site's welcome is centred, watch first.

Three standing rules went into CLAUDE.md: signed out, the phone is nobody's;
the fan's device is a watch; a reading without skin contact is not a beat.

**Not proven.** Whether the H10 on the table sent *contact = not detected*
(which the filter rejects) or something else. The next build settles it in a
minute: strap on the table, and setup must say "sem contato". If a number
still shows, the second guard is R-R presence.

---

## 2026-09-25 — the test round on b195, and everything it found, built in one batch

Felipe ran the rest of the 24/09 list on b195 the morning of 25/09 and asked
for the findings to be collected and fixed together (*"anote essas correções
que eu vou fazer os outros testes. Aí você arruma tudo de uma vez"*). The raw
notes, eighteen of them, were kept in the session; this is what they came to.

### What passed in a hand

- **Test 9, the reveal lock, completely.** A sealed night, *"Sua noite está
  sendo revelada"*, LACRADA in Você, the curve opening by itself at 10h, and
  *"Sua noite abriu."* in the notification shade at 10h01. The alarm is
  inexact and was one minute late, which is fine.
- **Test 8, marks without a night.** On the operator account at
  `/admin/eventos`: APITO recorded as the kick-off and a second APITO refused,
  GOL twice, and a mark tapped in airplane mode reaching the server with the
  next tap. Marks arrived during the capture, not at its end.
- **Test 1, the 6-digit code (#64), on the phone and on the site.** Wrong
  code (*"faltam 4 tentativas"*), the resend countdown, *"Só o mais novo
  vale"*, and the right code creating the account. The codes arrived by
  e-mail both times.
- **Instagram:** the card saved to the camera roll went into a Story as a
  sticker, and *Copiar card* pasted into the Story editor. Both roads work.

### What it found

1. **Another account's night was fully open.** Signed into his own account,
   Felipe opened *Teste 7B-2* — recorded by the test account — and saw the
   curve, made the card and could share it. The phone showed every night it
   held, whoever had recorded it. His read: another account's night must not
   be visible at all. **Decided: hidden, never deleted** (*"vamos seguir com
   a sua recomendação e manter escondida"*).
2. **Four "Sua noite abriu." for two nights that no longer existed.**
   Creating the test account at 09:38 wiped the phone's nights (open item 65)
...
3. **A night was recorded signed out.** The phone had been signed out
   ...
   account, did not go up when he signed in (only the next app start retries
   it), and meanwhile the event's feed told him *"Sua noite não chegou
   aqui"* about a night sitting on the same phone. Felipe's rule: **a night
   is recorded only with an account.**
4. **Nobody could tell why he was signed out.** He did not remember tapping
   Sair. The one line that would have told a Sair from a refused renewal was
   overwritten the moment he signed back in.
5. **"Só neste celular: o servidor não aceitou o cadastro"** was the whole
   explanation for an event that never reached the server. The likely cause
   was the signed-out phone, not a refusal, and the line could not say
   which. Under it, open item 52: the operator switches were a local flag
   any account could turn on.
6. **Setup on a new account** opened with Felipe's Polar H10 already
   "paired" (the pairing lives in the phone's prefs and nothing cleared it,
   the same leak as the photo on 24/09), with no way to search for a device,
   the after-a-night line *"Não achamos batida nessa janela"* where there was
   no window, no word that anything was done, and *"Fazer isso depois"* in
   grey. His ask: *"tem que aparecer um campo de procurar o dispositivo…
   depois de conectado, pronto, conexão feita, e um botão pra ir pra home."*
7. **The sealed night** offered *"Exportar sessão — operador"* to every fan,
   and its only way out was the small arrow.
8. **The site's welcome** (`/onboarding`) opened on a ❤️, asked to
   *"conectar seu wearable"* from a list with fitness emojis, and "connected"
   a watch by saving a row with the token `placeholder-token`, a Phase-0
   simulation that was never removed. When that failed it went on to *"Tudo
   pronto!"* anyway. The site cannot read a watch at all.
9. **Low:** with six networks, the card preview in the share menu shrinks to
   a sliver. This was predicted in the #96 entry. Not built.

### What was built — b196

- **Nights have an owner from the moment they exist.** `saveNight` records
  the recording account, not the upload. The phone keeps a *viewer*: the
  last account signed in, which survives Sair so the person who signed out
  still finds their nights. Every list (Você, the gallery, the public
  profile) shows the viewer's nights plus ownerless ones from before this
  rule. Another account's nights are hidden, and Você says how many are
  there instead of claiming *"Nenhum show ainda"*. Creating an account
  wipes nothing. **Deleting an account takes only that account's nights**,
  with their photos, events, raw capture and alarms. The sync never uploads
  another account's night under this one. A night opened by id that is gone
  or belongs to another account says so, with *Voltar pro início*.
- **Reveal alarms follow the account.** They are rescheduled on every
  sign-in. The receiver checks that the night still exists and is the
  viewer's before posting.
- **Capture needs a live session.** AO VIVO says *"Entra na sua conta pra
  gravar a noite"* with a Pink *Entrar* before any tap, and a declined tap
  says it again. Signing in (either door) sends any waiting night at once.
  The event feed, when this phone holds an unsent night of that event, says
  so and offers *"Mandar minha noite agora"*.
- **Item 52 is closed.** `/api/auth/me`'s `is_admin` is stored against the
  account (asked on sign-in and on every app start). The mark buttons and
  the event shortcuts need both the switch and the server's word. A fan
  sees *"Cadastrar eventos e marcar momentos é só pra conta de operador"*.
  The capture's local-only line says which of four things happened (no
  account, not an operator, offline, or the server's own sentence) and has
  *TENTAR DE NOVO*.
- **The phone remembers how the session ended.** *"Você saiu da conta em
  25.09.26 às 08h12"* or *"O servidor encerrou sua sessão em…"*, kept until
  the next sign-in.
- **Setup rebuilt** (`SetupScreen`), in three states. With nothing paired:
  a Pink *Procurar dispositivo* and a quiet *Fazer isso depois*. Searching:
  what was found, or an honest line (Bluetooth off, no permission, or
  nothing after 15 s). Paired: the app connects and waits for a beat.
  **Only a real beat earns "Pronto, conexão feita"**, with the live number
  as proof; until then the sensor is *"pareado"*, which is all the phone
  knows. A watch in Health Connect stays as the second, quieter road. A
  different account arriving now clears the sensor, the watch, the
  participant id and the marked event, like the photo.
- **The sealed night** has a Pink *Voltar pro início*. The raw export shows
  only on the operator's account or on a phone set up with a participant id,
  on the sealed screen and the open one alike.
- **The site's welcome tells the truth.** The wordmark, *"Sua conta está
  pronta"*, the night is recorded by the app on the phone, the app is in
  closed testing and Android only, and *Ver os eventos*. No emoji, no
  "wearable", no fake connection. The site's meta description lost
  "wearable" too.

### Found while building

**"Apagar conta" while signed out deleted the phone's data and claimed the
account was gone.** The code read a missing session as "never signed in,
nothing on the server to delete", but signed out is not never signed in. It
now asks to sign in first and deletes nothing. This is the log's usual class,
in the one place the privacy page makes a promise.

### Left open

- **The rows `placeholder-token` wrote to `wearable_connections`** are still
  in the database. The site profile lists them under *"Dispositivos
  conectados"*. They should be counted and deleted.
- **Bringing a night down from the server** is still the second half of
  item 65: what a new phone or a reinstall needs.
- **The *Trava da revela* switch is still on** on Felipe's phone from
  test 9. Every new night will be sealed until 10h until it is turned off.
- **The share-menu card preview** (finding 9).
- **Operator tools after the update need one online app start or sign-in**
  to learn the role. On a fresh install at the venue, sign in before going.
...
**Set the same evening, and the first reset mail arrived.** Felipe put the
key on Railway, asked again, and the mail reached his inbox from
`oi@mail.tumtum.cc`. Password recovery worked in production for the first
time since it was built on 26/08, and #98 was unblocked. The mail greeted
him *"Oi, Felipe Zanucci ."*, with a space before the full stop, because his
name had been stored as typed, trailing space and all. The greeting now
trims the name, and escapes it in the HTML part, since it is text a person
chose.
...
## 2026-09-24 — test 7 in a hand: 7a passed, and the steps for 7b cost the phone's nights

**7a passed.** Felipe made a new account (*teste1*) and opened the
TesteMadonna feed: *"Esse feed é de quem estava lá. Sua noite não chegou
aqui."* It showed that exact sentence, with no empty list, and no false
*"ninguém mostrou nada"*.

**7b and 7c could not run, and the fault was in the steps, not the app.**
The steps sent this afternoon had him make the test account through
*Configurações → Criar outra conta*. They then asked him to open, in *Você*,
a night uploaded by his usual account. But creating a second account
**wipes the phone's nights**. The screen said so before the tap (*"as noites
gravadas aqui somem do celular — elas ficam com a conta antiga no
servidor"*), and this log has said so since 18/09. *Você* came back empty:
*"Nenhum show ainda."* The steps stated something false about the app. That
is the class of defect this log counts in the product, this time in the
instructions for testing it. They were written from the card screen's code
without reading the account screen's. **A test step that goes through a
screen is written after reading that screen.**

What it cost: every night on Felipe's phone. As far as this log knows they
were all test nights (teste7, jogo1, show1, the Madonna test and the like).
The uploaded ones are **not lost**: each is on the server under the account
that uploaded it, and anything posted to a feed stays there. A night that had
never finished uploading is gone for good. But the app has no road to bring a night back (item 65), so they will
not reappear in *Você*. Realness lives in the other app, `cc.tumtum.capture`,
and was not touched.

7b has a road that works: *Entrar* with an existing account keeps the phone's
nights, which is how #58 arose on 23/09 (item 60 has the steps). 7c cannot
be run anymore: it needed a night with no owner, from before 23/09, and the
last of those left with the wipe.

**The same test found item 64**: `teste@teste.com` made an account. Felipe
wants only real addresses accepted. No validator can know that, and a code
by e-mail can. His call on where it blocks.

---

## 2026-09-24 — b189 in a hand: Status works, and "Salvar card" saved where nobody looks

Felipe tested #96 on b189 the same evening. **The grid with every network
looks right, and Status do WhatsApp worked end to end**: the button opened
Status, the card went up, and he saw it from another phone. So on his
WhatsApp the undocumented `status@broadcast` extra is honoured.

**One finding, and it is the project's usual class.** *Salvar card* said
*"Card salvo na galeria… em Imagens › TumTum"*, and the file was real, but
he found it in the **Files app** and not among his photos. For a card saved
*to be added to a Story later*, a place the photo picker does not open on is
as good as not saved. His rule: *"o card precisa ficar salvo no rolo da
câmera."*

Two things were wrong, and both are fixed:

- **The folder.** `Pictures/TumTum` is a folder of its own. Google Photos
  shows folders other than the camera's only under Library, and a Story's
  photo picker opens on recent photos. The card now goes to
  **`DCIM/Camera`**, where the camera writes, so it sits beside the person's
  own photos in every gallery. That mixes a TumTum image into the camera's
  folder, and that is the request: the roll is where a person looks.
  `DCIM/TumTum` would have been tidier and would have repeated the bug on
  Google Photos.
- **The entry itself.** The image was inserted and then written, with no
  pending flag and no date taken. A gallery could index it before the bytes
  arrived (size and dimensions zero, which some galleries skip) and sort it
  by a missing date, at 1970, far below today. Now the entry is **pending**
  while written and released after, carries **DATE_TAKEN = now** and its
  dimensions, and a failed write deletes its half-made entry rather than
  leaving a blank thumbnail.

The message says where it went and what it is for: *"Card salvo nas fotos da
câmera, com fundo transparente. É só escolher ele quando for postar."*
Cards saved by b189 stay in Pictures/TumTum; nothing moves them.

**To confirm in a hand:** the saved card appears first in the gallery's
photos and in Instagram's Story picker, and its transparent background
survives being picked there.

---

## 2026-09-24 — the share menu: Pink, no hole, and Status

Felipe asked for items 61–63 together, after the day's five networks worked
on b187. They are one screen, so they became one change.

- **No hole (#61).** The networks sat two to a row and an odd one kept half
  the width, so WhatsApp stood beside an empty cell. `ShareGrid.rows` now
  pairs them in order, and a button without a partner takes the whole row.
  A label that does not fit in half a row also takes the whole row, and
  whether it fits is **measured** with the button's own type, not guessed:
  *"Status do WhatsApp"* fits beside WhatsApp on a 384 dp phone and does not
  on a 360 dp one, and a wrapped label inside a 56 dp button is worse than a
  taller menu. Tested without a device (`ShareGridTest`, every 6-button
  pattern: order kept, never more than two, a pair only of labels that fit).
- **Pink (#62).** The networks are TumTum Pink with black type (7.93:1),
  equal among themselves. Copiar card, Salvar card and Mais apps are small
  outlined chips beneath, sized to their labels and wrapping onto a second
  line on a narrow phone. The eye lands on *where does this go* first. On
  Felipe's phone the chips' one row is shorter than the two rows of 56 dp
  buttons it replaces, so the card preview above keeps a little more room even
  with a sixth network. On a 360 dp phone, where Status takes its own row and
  the chips wrap, the menu is taller and the preview smaller.
- **Status do WhatsApp (#63).** WhatsApp has no Status intent for other
  apps. The known road is an undocumented extra, `jid` = `status@broadcast`,
  on the same share intent as the chat button. **Nothing comes back to say
  whether WhatsApp honoured it**: if it ignores the extra, the person lands in
  the chat picker. So the app does not claim Status. It says both roads
  before the tap, in a footnote under the menu (*"se abrir a lista de
  conversas, toca em Meu status, lá no topo"*), which is where the eye is at
  the only moment it can help. An outright refusal (the intent throws) opens
  the chat picker and says so on return. The file is the one WhatsApp gets:
  the card burned into the video, at most 30 s, inside Status's limit.

**What only the phone can answer:** whether `status@broadcast` opens Status
on Felipe's WhatsApp, and whether the picker's first row reads *Meu status*.
If the extra does nothing on his version, the button still lands one tap
from Status and the footnote is what makes that true.

Compiled only in CI, like everything on this app. `ShareGrid` and its test
are plain Kotlin. The Compose half uses `rememberTextMeasurer` and
`FlowRow`, both in the pinned Compose BOM (2024.12.01).
